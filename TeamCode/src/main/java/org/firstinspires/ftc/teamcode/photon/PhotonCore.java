...
public class PhotonCore implements Runnable, OpModeManagerNotifier.Notifications {
    ...
    public static void photonInit(int maximumParallelCommands, LynxModule.BulkCachingMode mode, boolean PARALLELIZE_SERVOS){
        ...
        PhotonCore.CONTROL_HUB.setBulkCachingMode(mode);
        PhotonCore.EXPANSION_HUB.setBulkCachingMode(mode);
        PhotonCore.experimental.setMaximumParallelCommands(maximumParallelCommands); // Can be adjusted based on user preference - but raising this number further can cause issues
        PhotonCore.PARALLELIZE_SERVOS = PARALLELIZE_SERVOS;
        PhotonCore.enable();
    }
    ...
    public static Commands.Command clearBulkCacheCommand = new Commands.ContinuousCommand(PhotonCore::clearBulkCache);
