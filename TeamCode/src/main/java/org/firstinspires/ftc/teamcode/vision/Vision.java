...
public class Vision {
    ...
    public List<ArtifactDescriptor> getArtifactDescriptors(Pose botPose){
    ...
    public Pose getBotPoseMT1(){
        ...
        double llHeading = llPose.getOrientation().getYaw(AngleUnit.DEGREES);

        double xTransformed = llY + 72;
        double yTransformed = -llX + 72;
        double yawTransformed = (llHeading + 270) % 360;
        if (yawTransformed < 0) {
            yawTransformed += 360;
        }
    ...
    public Pose getBotPoseMT2(GoBildaPinpointDriver pinpoint){
        if (!limelight.isRunning()){
            limelight.start();
        }
        limelight.pipelineSwitch(APRIL_TAGS_PIPELINE_INDEX);
    ...
    public Pose getBotPoseMT2WithMT1() {
        ...
        if (botPoseMT1 != null) {
            double yaw = botPoseMT1.getHeading();

            telemetry.addData("yaw from mt1", yaw);
    ...
    // TODO: create a limelight coordinate system to pedro and the inverse function
}
